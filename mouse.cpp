...
VOID MouseUpdate(VOID)
{
	...
	//左ボタンを押しているかチェック（TRUEは０以外なので、２も４もTRUE）
	...
	//中ボタンを押しているかチェック（TRUEは０以外なので、２も４もTRUE）
	...
	//右ボタンを押しているかチェック（TRUEは０以外なので、２も４もTRUE）
...
/// </summary>
/// <param name="MOUSE_INPUT_">MOUSE_INPUT_</param>
/// <returns>場単を押し上げているならTRUE</returns>
...
{
	float MilliSec = 1000.0f;		//1秒が1000ミリ秒

	//押し続ける時間 = 秒数×FPS値
	//例）６０FPSのゲームで、1秒間押し続けるなら、1秒×６０FPS
...
BOOL MouseClick(int MOUSE_INPUT_)
{
	...
	{
		return FALSE;	//マウスをクリックして否、押し続けている
	}
}
