...
extern BOOL MouseUp(int MOUSE_IINPUT_);		//ボタンを押しているか、マウスコードで判断する
extern BOOL MouseDownKeep(int MOUSE_INPUT_, int milliTime);	//ボタンを押しているか、マウスコードで判断する
extern BOOL MouseClick(int MOUSE_INPUT_);	//マウスをクリックしたか、マウスコードで判断する
extern VOID MouseDraw(VOID);				//マウスの情報を描画する
extern BOOL MouseMaruClick(MARU rect, int MOUSE_INPUT_);	//マウスが矩形領域でクリックしたか？
